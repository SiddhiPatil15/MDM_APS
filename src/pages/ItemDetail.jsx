...
import { MapPin, User, Calendar, ShieldCheck, MessageSquare, CheckCircle, XCircle } from 'lucide-react';
...
const ItemDetail = () => {
  ...
  const { items, requestItem, currentUser, requests, handleRequest } = useAppContext();

  const item = items.find(i => i.id.toString() === id);
  const isOwner = currentUser && currentUser.id === item?.ownerId;
  const itemRequests = requests.filter(req => req.itemId === item?.id.toString() && req.status === 'Pending');
          ...
          <div className="detail-actions">
            {isOwner ? (
              <div className="owner-requests-section">
                <h3>Pending Requests for your item</h3>
                {itemRequests.length > 0 ? (
                  itemRequests.map(req => (
                    <div key={req.id} className="request-card" style={{marginTop: '1rem', padding: '1rem', border: '1px solid var(--border-color)', borderRadius: '8px', backgroundColor: 'var(--card-bg)'}}>
                      <p><strong>{req.requesterName}</strong> wants this item.</p>
                      <p style={{fontStyle: 'italic', marginBottom: '1rem'}}>"{req.message}"</p>
                      <div style={{display: 'flex', gap: '1rem'}}>
                        <button className="btn btn-primary btn-sm" onClick={() => handleRequest(req.id, 'Accepted')}><CheckCircle size={16} /> Approve</button>
                        <button className="btn btn-outline btn-sm" onClick={() => handleRequest(req.id, 'Declined')}><XCircle size={16} /> Decline</button>
                      </div>
                    </div>
                  ))
                ) : (
              ...
              <button className="btn btn-secondary full-width-btn" disabled>
                Item Already Claimed
              </button>
            ) : (
              <button className="btn btn-primary full-width-btn" onClick={handleRequestAction}>
                Request This Item
              </button>
            )}
          </div>
